...
data "coder_parameter" "repo_url" {
  ...
  description  = "NetBox repository URL to clone into ~/work on first start."
  default      = "https://github.com/netbox-community/netbox/tree/v4.6.0"
  mutable      = false
  order        = 10
}

data "coder_parameter" "plugin_repo_url" {
  name         = "plugin_repo_url"
  display_name = "Plugin Repository"
  description  = "NetBox plugin repository to clone into ~/plugins on first start."
  default      = "https://github.com/Onemind-Services-LLC/netbox-metatype-importer"
  mutable      = false
  order        = 20

  validation {
    regex = "^$|^(https?://|ssh://|git@|git://)[a-zA-Z0-9._/:@~-]+$"
    error = "Must be a valid Git repository URL (https, git@, ssh://, or git://) without special characters."
  }
}

data "coder_parameter" "plugin_base_branch" {
  name         = "plugin_base_branch"
  display_name = "Plugin Base Branch"
  description  = "Remote branch used when creating the local plugin development branch."
  default      = "dev"
  mutable      = true
  order        = 21
  ...
  validation {
    regex = "^[^\\s]+$"
    error = "Plugin base branch cannot be empty or contain whitespace."
  }
}

data "coder_parameter" "plugin_work_branch" {
  name         = "plugin_work_branch"
  display_name = "Plugin Work Branch"
  description  = "Local plugin branch checked out for development."
  default      = "netbox-4.6-compat"
  mutable      = true
  order        = 22

  validation {
    regex = "^[^\\s]+$"
    error = "Plugin work branch cannot be empty or contain whitespace."
  }
}
...
data "coder_parameter" "enable_postgres" {
  ...
  description  = "Run the private CloudNativePG PostgreSQL service required by NetBox."
  type         = "bool"
  default      = "true"
  mutable      = true
  order        = 210
...
data "coder_parameter" "enable_redis" {
  name         = "enable_redis"
  display_name = "Redis"
  description  = "Run the private Redis service required by NetBox tasks and caching."
  type         = "bool"
  default      = "true"
  mutable      = true
  order        = 220
...
data "coder_external_auth" "github" {
  count = can(regex("^https://github\\.com/", data.coder_parameter.repo_url.value)) || can(regex("^https://github\\.com/", data.coder_parameter.plugin_repo_url.value)) ? 1 : 0
...
locals {
  template_name       = "netbox"
  workspace_namespace = "coder-workspaces"
  # renovate: datasource=docker depName=ghcr.io/abhi1693/home-lab versioning=regex:^netbox-(?<patch>\d{2})(?<minor>\d{2})(?<major>\d{4})$
  workspace_image = "ghcr.io/abhi1693/home-lab:netbox-16052026"
  ...
  plugin_root_dir          = "${local.workspace_home}/plugins"
  plugin_repo_name         = "netbox-metatype-importer"
  plugin_repo_dir          = "${local.plugin_root_dir}/${local.plugin_repo_name}"
  plugin_module_name       = "netbox_metatype_importer"
  jetbrains_project_dir    = local.workspace_home
  ...
  netbox_running           = data.coder_workspace.me.start_count != 0
  ...
  netbox_port              = 8000
  netbox_admin_username    = local.owner_name
  netbox_admin_email       = "${local.owner_name}@coder.local"
  agent_preserve_env = join(",", [
    ...
    "NETBOX_DEBUG",
    "NETBOX_DEVELOPER",
    "NETBOX_METATYPE_GITHUB_TOKEN",
    "NETBOX_PORT",
    "NETBOX_PLUGIN_BASE_BRANCH",
    "NETBOX_PLUGIN_DIR",
    "NETBOX_PLUGIN_MODULE",
    "NETBOX_PLUGIN_REPO_URL",
    "NETBOX_PLUGIN_WORK_BRANCH",
    "NETBOX_SECRET_KEY",
    "PGDATABASE",
    "PGHOST",
    "PGPASSWORD",
...
def page():
    netbox_rows = [
        row("Status", "NETBOX_PORT", "enabled"),
        row("URL", "NETBOX_PORT", f'http://127.0.0.1:{value("NETBOX_PORT", "8000")}'),
        row("Username", "NETBOX_ADMIN_USERNAME", value("NETBOX_ADMIN_USERNAME")),
        row("Email", "NETBOX_ADMIN_EMAIL", value("NETBOX_ADMIN_EMAIL")),
        row("Password", "NETBOX_ADMIN_PASSWORD", value("NETBOX_ADMIN_PASSWORD"), True),
        row("Command", "NETBOX_PORT", f'cd ~/work/netbox && ~/.venv/netbox/bin/python manage.py runserver 0.0.0.0:{value("NETBOX_PORT", "8000")} --insecure'),
    ]
    plugin_rows = [
        row("Module", "NETBOX_PLUGIN_MODULE", value("NETBOX_PLUGIN_MODULE")),
        row("Path", "NETBOX_PLUGIN_DIR", value("NETBOX_PLUGIN_DIR")),
        row("Repository", "NETBOX_PLUGIN_REPO_URL", value("NETBOX_PLUGIN_REPO_URL")),
        row("Base branch", "NETBOX_PLUGIN_BASE_BRANCH", value("NETBOX_PLUGIN_BASE_BRANCH")),
        row("Work branch", "NETBOX_PLUGIN_WORK_BRANCH", value("NETBOX_PLUGIN_WORK_BRANCH")),
        row("Install", "NETBOX_PLUGIN_DIR", f'~/.venv/netbox/bin/python -m pip install -e "{value("NETBOX_PLUGIN_DIR")}"'),
        row("Tests", "NETBOX_PLUGIN_MODULE", f'cd ~/work && ~/.venv/netbox/bin/python netbox/manage.py test {value("NETBOX_PLUGIN_MODULE")}'),
    ]
    ...
    body = (
        section("NetBox", netbox_rows)
        + section("Plugin", plugin_rows)
        + section("PostgreSQL", pg_rows)
        + section("Redis", redis_rows)
    )
...
resource "coder_script" "netbox_environment" {
  count = local.netbox_running ? 1 : 0

  agent_id           = coder_agent.main.id
  display_name       = "NetBox Setup"
  run_on_start       = true
  ...
  script = <<-EOT
    ...
    repo_dir=${jsonencode(local.workspace_repo_dir)}
    netbox_dir="$repo_dir/netbox"
    venv_dir="$HOME/.venv/netbox"
    state_dir="$HOME/.cache/coder/netbox"
    setup_marker="$state_dir/setup-complete"

    mkdir -p "$state_dir" "$HOME/.venv"
    rm -f "$setup_marker"

    if [ -z "$${PGHOST:-}" ] || [ -z "$${REDIS_HOST:-}" ]; then
      echo "NetBox requires the PostgreSQL and Redis workspace services to be enabled." >&2
      exit 1
    fi

    plugin_repo_url="$${NETBOX_PLUGIN_REPO_URL:-}"
    plugin_base_branch="$${NETBOX_PLUGIN_BASE_BRANCH:-dev}"
    plugin_work_branch="$${NETBOX_PLUGIN_WORK_BRANCH:-netbox-4.6-compat}"
    plugin_dir="$${NETBOX_PLUGIN_DIR:-$HOME/plugins/netbox-metatype-importer}"
    plugin_module="$${NETBOX_PLUGIN_MODULE:-netbox_metatype_importer}"
    ...
    clone_or_update_plugin() {
      ...
      if [ ! -d "$plugin_dir/.git" ]; then
        if ! git clone --branch "$plugin_base_branch" "$plugin_repo_url" "$plugin_dir"; then
          rm -rf "$plugin_dir"
          git clone "$plugin_repo_url" "$plugin_dir"
        fi
      fi

      git -C "$plugin_dir" remote set-url origin "$plugin_repo_url"
      git -C "$plugin_dir" fetch origin --prune || true

      if git -C "$plugin_dir" status --porcelain | grep -q .; then
        echo "Plugin checkout has local changes; leaving current branch untouched."
        return 0
      fi

      if git -C "$plugin_dir" show-ref --verify --quiet "refs/remotes/origin/$plugin_work_branch"; then
        git -C "$plugin_dir" switch "$plugin_work_branch" 2>/dev/null || git -C "$plugin_dir" switch -c "$plugin_work_branch" "origin/$plugin_work_branch"
      elif git -C "$plugin_dir" show-ref --verify --quiet "refs/heads/$plugin_work_branch"; then
        git -C "$plugin_dir" switch "$plugin_work_branch"
      elif git -C "$plugin_dir" show-ref --verify --quiet "refs/remotes/origin/$plugin_base_branch"; then
        git -C "$plugin_dir" switch -c "$plugin_work_branch" "origin/$plugin_base_branch"
      else
        git -C "$plugin_dir" switch -c "$plugin_work_branch"
      fi
    }

    apply_metatype_importer_netbox46_patch() {
      if [ "$plugin_module" != "netbox_metatype_importer" ] || [ ! -d "$plugin_dir/netbox_metatype_importer" ]; then
        return 0
      fi
...
config_path = root / "netbox_metatype_importer" / "__init__.py"
utils_path = root / "netbox_metatype_importer" / "utils.py"

if config_path.exists():
    text = config_path.read_text(encoding="utf-8")
    text = re.sub(r"min_version = '[^']+'", "min_version = '4.6.0'", text)
    text = re.sub(r"max_version = '[^']+'", "max_version = '4.6.99'", text)
    config_path.write_text(text, encoding="utf-8")

if utils_path.exists():
    text = utils_path.read_text(encoding="utf-8")
    marker = "('front-ports', forms.FrontPortTemplateImportForm),"
    addition = "            ('port-mappings', forms.PortTemplateMappingImportForm),"
    if "PortTemplateMappingImportForm" not in text and marker in text:
...
PY
    ...
    for _ in $(seq 1 180); do
      if [ -f "$netbox_dir/manage.py" ] && [ -f "$repo_dir/requirements.txt" ]; then
        break
      fi
      sleep 2
    done

    if [ ! -f "$netbox_dir/manage.py" ]; then
      echo "NetBox repository was not cloned into $repo_dir before setup timed out." >&2
      exit 1
    fi
    ...
    requirements_hash="$(sha256sum "$repo_dir/requirements.txt" | awk '{print $1}')"
    installed_hash="$(cat "$state_dir/requirements.sha" 2>/dev/null || true)"
    if [ "$installed_hash" != "$requirements_hash" ]; then
      "$venv_dir/bin/python" -m pip install -r "$repo_dir/requirements.txt"
      "$venv_dir/bin/python" -m pip install ruff pre-commit
      echo "$requirements_hash" > "$state_dir/requirements.sha"
    fi

    if [ -n "$plugin_repo_url" ]; then
      "$venv_dir/bin/python" -m pip install -e "$plugin_dir"
    fi

    cat > "$netbox_dir/netbox/configuration.py" <<'PY'
import os

ALLOWED_HOSTS = [host.strip() for host in os.environ.get("NETBOX_ALLOWED_HOSTS", "*").split(",") if host.strip()]
...
SECRET_KEY = os.environ["NETBOX_SECRET_KEY"]
API_TOKEN_PEPPERS = {1: os.environ["NETBOX_API_TOKEN_PEPPER"]}
DEBUG = os.environ.get("NETBOX_DEBUG", "false").lower() == "true"
DEVELOPER = os.environ.get("NETBOX_DEVELOPER", "false").lower() == "true"
PLUGIN_MODULE = os.environ.get("NETBOX_PLUGIN_MODULE", "")
PLUGINS = [PLUGIN_MODULE] if PLUGIN_MODULE else []
PLUGINS_CONFIG = {}
if PLUGIN_MODULE == "netbox_metatype_importer":
    PLUGINS_CONFIG["netbox_metatype_importer"] = {
        "repo_owner": "netbox-community",
        "repo": "devicetype-library",
        "branch": "master",
        "github_token": os.environ.get("NETBOX_METATYPE_GITHUB_TOKEN", ""),
    }
PY
...
User = get_user_model()
username = os.environ["NETBOX_ADMIN_USERNAME"]
password = os.environ["NETBOX_ADMIN_PASSWORD"]
email = os.environ.get("NETBOX_ADMIN_EMAIL", "")
...
resource "coder_script" "netbox_server" {
  count = local.netbox_running ? 1 : 0

  agent_id     = coder_agent.main.id
  display_name = "NetBox Server"
  run_on_start = true
  ...
  script = <<-EOT
    ...
    state_dir="$HOME/.cache/coder/netbox"
    setup_marker="$state_dir/setup-complete"
    pid_file="$state_dir/runserver.pid"
    log_file="$state_dir/runserver.log"

    for _ in $(seq 1 600); do
      if [ -f "$setup_marker" ] && [ -x "$venv_dir/bin/python" ]; then
        break
      fi
      sleep 1
    done
    ...
    if [ ! -f "$setup_marker" ]; then
      echo "NetBox setup did not complete before server startup timed out." >&2
      exit 1
    fi

    if curl -fsS "http://127.0.0.1:${local.netbox_port}/login/" >/dev/null 2>&1; then
      exit 0
    fi

    rm -f "$pid_file"
    cd "$netbox_dir"
    nohup "$venv_dir/bin/python" manage.py runserver "0.0.0.0:${local.netbox_port}" --insecure --noreload >"$log_file" 2>&1 &
    echo "$!" > "$pid_file"
...
resource "coder_app" "netbox" {
  count = local.netbox_running ? 1 : 0

  agent_id     = coder_agent.main.id
  display_name = "NetBox"
  slug         = "netbox"
  url          = "http://127.0.0.1:${local.netbox_port}"
  icon         = "/icon/python.svg"
  order        = 30
  subdomain    = false

  healthcheck {
    url       = "http://127.0.0.1:${local.netbox_port}/login/"
    interval  = 5
    threshold = 12
  }
}
  ...
  spec {
    ...
    container {
      ...
      env {
        ...
        value = local.netbox_admin_email
      }
      ...
      env {
        ...
        value = local.netbox_admin_username
      }
      ...
      env {
        ...
        value = random_password.netbox_api_token_pepper.result
      }
      ...
      env {
        name  = "NETBOX_DEVELOPER"
        value = "true"
      }

      env {
        name  = "NETBOX_METATYPE_GITHUB_TOKEN"
        value = ""
      }

      env {
        name  = "NETBOX_PORT"
        value = tostring(local.netbox_port)
      }

      env {
        name  = "NETBOX_PLUGIN_BASE_BRANCH"
        value = data.coder_parameter.plugin_base_branch.value
      }

      env {
        name  = "NETBOX_PLUGIN_DIR"
        value = local.plugin_repo_dir
      }

      env {
        name  = "NETBOX_PLUGIN_MODULE"
        value = local.plugin_module_name
      }

      env {
        name  = "NETBOX_PLUGIN_REPO_URL"
        value = data.coder_parameter.plugin_repo_url.value
      }

      env {
        name  = "NETBOX_PLUGIN_WORK_BRANCH"
        value = data.coder_parameter.plugin_work_branch.value
      }
...
resource "random_password" "netbox_api_token_pepper" {
  length  = 64
  special = false
}
